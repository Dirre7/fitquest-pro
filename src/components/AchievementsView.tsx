      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sortedAndFilteredAchievements.map((ach) => {
          ...
          return (
            <div
              key={ach.id}
              className={`rounded-3xl p-5 sm:p-6 border transition-all flex flex-col justify-between relative overflow-hidden group ${getTierCardBorder(
                ach.tier,
                ach.unlocked
              )}`}
            >
              ...
              <div>
                ...
                <div className="flex items-center justify-between gap-3 mb-4">
                  ...
                  >
                    {ach.unlocked ? getIcon(ach.icon) : ach.currentProgress > 0 ? getIcon(ach.icon) : <Lock className="w-5 h-5" />}
                  </div>
